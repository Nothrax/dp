...
LoraEndpoint::LoraEndpoint(const std::shared_ptr<structures::GlobalContext> &context): Endpoint(context) {
	receiveBuffer_ = std::vector<uint8_t>(sizeof(input_protocol::InputProtocolMessage));
}
...
std::shared_ptr<input_protocol::InputProtocolMessage> LoraEndpoint::getMessage(unsigned int timeoutMs) {
	std::shared_ptr<input_protocol::InputProtocolMessage> message { nullptr };
	if(!initialized_) {
	...
	message = std::make_shared<input_protocol::InputProtocolMessage>();

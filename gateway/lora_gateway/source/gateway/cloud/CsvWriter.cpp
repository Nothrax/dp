...
void CsvWriter::write(const gateway::structures::LoraMessage &data) {
	...
	file_ << std::to_string(data.protocolVersion) << "," << std::to_string(data.unitNumber) << "," << std::to_string(data.flags) << "," << floatValues[0] << "," << floatValues[1] << "," << data.values[2] << "," << std::to_string(data.checkSum) << "\n";

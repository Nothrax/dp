...
void Logger::initLogger(std::string logPath, bool verbose) {
    try
    {
        ...
        auto console_sink = std::make_shared<spdlog::sinks::rotating_file_sink_mt>(logPath + "LoRaGateway_error_log.txt", 1024*1024*5, 1, false);
        console_sink->set_level(spdlog::level::warn);
        sinks.push_back(console_sink);

        //rotating file sink for all logs
        sinks.push_back(std::make_shared<spdlog::sinks::rotating_file_sink_mt>(logPath + "LoRaGateway_info_log.txt", 1024*1024*5, 1, false));
